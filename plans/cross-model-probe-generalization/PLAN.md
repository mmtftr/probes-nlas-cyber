...
from one 8 h GH200 window on Clariden.
...
Orchestrator + cluster mechanics: `src/remotes/clariden/README.md`. Account
`lsaie-ss26`, partition `debug`, single-node jobs (90 node-min budget), 4 GPUs
...
- **Inputs:** roster `src/remotes/clariden/models.txt` (dense ≤32B + small MoE);
