# [ai-generated]
"""exp-28 stretch copy of exp-26's pv_family.py — ONE change: INCREMENTAL probe
checkpointing. The original checkpoints only after ALL probes train; a gemma-12b
run needs ~85 min just for training, so a 90-min wall (or node failure) loses
everything and a resubmit loops forever. This copy (a) saves the probe npz after
EVERY trained probe, (b) on resume trains whichever feasible CWEs are missing
from the checkpoint instead of silently proceeding with a partial set.
Methodology (recipe, eval, bootstrap) is byte-identical to exp-26.
...
def within_pv(st, epochs, device, seed, pretrained=None, save_cb=None, logits_cache=None):
    ...
    # INCREMENTAL train/resume (exp-28 patch): start from whatever probes are
    # checkpointed, train only the missing feasible CWEs, checkpoint after EACH.
    if pretrained is not None:
        W, b = pretrained
        W = {c: W[c] for c in feasible if c in W}; b = {c: b[c] for c in feasible if c in b}
        print(f"[within] resuming with {len(W)} cached probes", file=sys.stderr)
    else:
        W, b = {}, {}
    for c in feasible:
        if c in W:
        ...
        if save_cb is not None:                 # CHECKPOINT after every probe
            save_cb(W, b)
    if save_cb is not None and W:
