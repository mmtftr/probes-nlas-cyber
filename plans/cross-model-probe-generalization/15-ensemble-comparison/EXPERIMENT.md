...
2. **Inputs**
   - *Models* (one per GPU) + best layers: `Qwen/Qwen2.5-Coder-32B-Instruct` L25,
     `google/gemma-3-27b-it` L19, `Qwen/Qwen3-32B` L27, `Qwen/Qwen3.6-27B` L30.
   - *Dataset* — SVEN before/after `./data/dataset.jsonl` (rebuilt 2026-06-01).
   ...
   - *Probe side* — `probe_members_scorer.py` at the best layer on CACHED acts
     (`./runs/layersweep_<slug>/acts`); trains `general`, `memory`, `injection`,
   ...
   - *Verbalized side* — `14-.../prompt_variants_judge.py` P(yes) at the first
     ...
     Shards SHARED with exp-14 in `./runs/promptsweep_<slug>/`; resumable.
...
- Files:
  ...
  - `run_ensemble_node.sh` (one model): (i) verbalized judge for ALL
    variants on the available GPUs (resumable, fills new prompts + Qwen3.6 gpu0),
    (ii) probe member scorer on 1 GPU, (iii) build_matrix. skip-if-exists on the matrix json.
...
- Preflight (Qwen3): read the per-VARIANT debug print in the run log — verify the
