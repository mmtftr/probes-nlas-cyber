...
Mirrors the overnight per-layer logic EXACTLY (it reuses the same functions):
  - SVEN group split via src/remotes/clariden/train_eval.load_or_make_split
...
exist. With --n-gpus G --gpu-id i, a worker handles layers where li % G == i, so
the node's 4 GPUs split the 62 layers. Baselines are NOT computed here (the
aggregator does them once) — this stays per-layer.
"""
